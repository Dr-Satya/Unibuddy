...
class AuthManager:
    ...
    def create_user(self, username: str, email: str, password: str) -> User:
        """Create a new user, respecting bcrypt 72-byte limit."""
        # Ensure password is at most 72 bytes when UTF-8 encoded (safe truncate)
        password_bytes = password.encode("utf-8")
        if len(password_bytes) > 72:
            password_bytes = password_bytes[:72]
        password = password_bytes.decode("utf-8", "ignore")  # Decode safely, ignoring any truncated bytes
    ...
    def register_user(self, username: str, email: str, password: str, 
        ...
        try:
            ...
            if 'user' in locals() and user is not None and user.id in self.users:
